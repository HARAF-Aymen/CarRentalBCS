...
def get_locations_acceptees_sans_contrat():
    """
    Retourne les demandes de location acceptées sans contrat généré,
    avec détails utilisateur.nom et vehicule.modele
    ...
    locations = (
        ...
        .outerjoin(ContratLocation, DemandeLocation.id == ContratLocation.location_id)
        .filter(ContratLocation.id == None)
        .all()
    )
    ...
    for loc in locations:
        mission = loc.mission
        utilisateur = mission.utilisateur
